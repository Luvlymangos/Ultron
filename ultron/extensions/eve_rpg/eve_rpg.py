...
class EveRpg:
    ...
    async def new_ship(self, player):
        ...
        if ship_tier is 1:
            ship = await self.weighted_choice([('Rifter', 25), ('Slicer', 25), ('Ultron', 5), ('Dramiel', 5)])
        elif ship_tier is 2:
            ship = await self.weighted_choice([('Ultron', 15), ('Dramiel', 15), ('Thrasher', 15), ('Catalyst', 15),
    ...
    async def ship_attributes(self, player, supplied_ship=None):
        ...
        if ship == 'Dramiel' or ship == 'Ultron':
